	  ...
	  if (found.length != 0)
	    {
	      ...
	      // remember to get selectors out first
	      // then read compressed data
	      // need to decide on a data structure for this!!! where do selectors go and how many bytes used?!
	      //int length = compressor->decode(thislist, postings + found.start, postings + found.start + found.length, selector...)
